...
function Orders({ params }: { params: { id: string } }) {
	...
	const userOrdersFiltered = [...userOrders]
		.filter((order: any) => order.sellerId === Number(id))
		.reverse();
	return (
								...
								<li key={order.id} className="mb-4 border border-gray-200 rounded-lg p-4 bg-white">
									...
									{order.status === "PENDIENTE" && (
										<div>
											...
											<button
												onClick={() =>
													handleAcceptOrder(order.id, order.products, order.sellerId, order.userId)
												}
												className="text-white p-2 bg-green-500 rounded-md m-2">
												Aceptar
											</button>
											<button
												onClick={() =>
													handleRejectOrder(order.id, order.products, order.sellerId, order.userId)
												}
												className="text-white p-2 bg-red-500 rounded-md m-2">
												Rechazar
											</button>
										</div>
									)}
									{order.status === "RECHAZADO" && (
										<div>
											...
											<button
												...
												className="text-white p-2 bg-blue-500 rounded-md m-2">
												Aceptar pedido
											</button>
										</div>
									)}
									{order.status === "ACEPTADO" && (
										<div>
											...
											<button
												onClick={() =>
													handlePrepareOrder(order.id, order.products, order.sellerId, order.userId)
												}
												className="text-white p-2 bg-blue-500 rounded-md m-2">
												Preparar
											</button>
									...
									{order.status === "PREPARANDO" && (
										<div>
											...
											<button
												onClick={() =>
													handleCookOrder(order.id, order.products, order.sellerId, order.userId)
												}
												className="text-white p-2 bg-yellow-500 rounded-md m-2">
												Listo para retirar
											</button>
											<button
												onClick={() =>
